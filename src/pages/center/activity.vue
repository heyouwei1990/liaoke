<template>
  <div class="activity-conponent">
    <div class="title">我的动态</div>
    <div class="edit-warp">
      <textarea class="textarea" id='editor' v-model="textareaV" placeholder="此时此刻，想和大家分享什么？" maxlength="250">
        <img src="http://chatfile.oss-cn-shanghai.aliyuncs.com/user/6590682/photos/icon_tan_close.png" alt="">
      </textarea>
      <div class="list-label" v-if="fileImg.length>0">图片({{fileImg.length}}/9)</div>
      <ul class="file-list" v-if="fileImg.length>0">
        <li v-for="(item,index) in fileImg" :key="index">
          <img :src="item" alt="">
          <i class="icon-cancel" @click="fileImg.splice(index,1)"></i>
        </li>
      </ul>
      <div class="tar">
        <label class="btn-file" @mouseover="uploadPhotos">
          <!-- <input type="file" accept="image/*" multiple> -->
        </label>
        <button class="btn-submit" @click="addlist">发布</button>
      </div>
    </div>
    <div class="activity-dl" v-if="activityList.length>0" v-for="(item,index) in activityList" :key="item.id">
      ...
      <div class="activity-dd">
        ...
        <div class="clearfix">
          ...
          <div class="ctrl-icon">
            ...
            <i class="icon-msg" @click="showTextarea(index,item.created_by_user)"></i>
            <i class="icon-delete" @click="deleteDynamic(item.id,index)"></i>
          </div>
        </div>
        <ul class="avatar-list" v-if="item.likes.length>0">
          <li v-for="(like,likeIndex) in item.likes" :key="likeIndex">
            <router-link
              :to="{name:'Zone',params:{uid:like.uid}}"
              target="_blank"
            >
        ...
        <ul class="comment-list" v-if="item.comment_list.length>0">
          <li v-for="(comment,cIndex) in item.comment_list" :key="comment.id" v-if="cIndex<item.max_show_comment">
            <div class="inline-block">
              <router-link
                class="nickname"
                :to="{name:'Zone',params:{uid:comment.created_by_user.uid}}"
                target="_blank"
              >
                {{comment.created_by_user.nickname}}
              </router-link>
              <span class="reply" v-if="comment.to_uid != 0">回复</span>
              <router-link
                class="nickname"
                v-if="comment.to_uid != 0"
                :to="{name:'Zone',params:{uid:comment.to_uid}}"
                target="_blank"
              >
                {{comment.to_user.nickname}}
              </router-link>
              <span class="colon">：</span>
              <span class="desc" v-html="comment.content"></span>
              <div class="ctrl-msg">
                <i v-if="hostUser && comment.created_by_user.uid!=hostUser.uid" class="icon-reply" @click="showTextarea(index,comment.created_by_user)"></i>
                <i v-if="hostUser" @click="deleteComment(comment.id,item.id,index)" class="icon-delete"></i>
              </div>
            </div>
          </li>
        </ul>
        ...
        <div class="comment-area" :class="{active:textareaIndex==index}">
          <div class="input" @click.stop="showTextarea(index,item.created_by_user)">评论</div>
    ...
    <div class="no-data" v-if="activityList.length==0 && !loading">
      <span>还未发布任何动态~ 快来发布吸引关注吧~</span>
    </div>
...
export default {
  ...
  methods: {
      ...
      this.fileObj = uploader(
        ...
        name => {
          ...
          for (let i = 0; i < this.fileImg.length; i++) {
            if (name === this.fileImg[i]) {
              index = i;
            }
          }
          if (index == -1) {
            // this.fileImg.push(name);
            // this.fileImg.push(name);
            if(this.fileImg.length<9){
              this.fileImg.push(name);
            }

            //  this.fileImg.splice(0,9);
          }
        }
      );
      this.fileObj.init();
    ...
    dataInit() {
      this.userId = this.hostUser ? Number(this.hostUser.uid) : null;
    ...
    addlist() {
      ...
      // console.log(photos);
      this.$get("/h5/dynamic/create", {
        content: this.textareaV,
        photos: photos,
        video_url: "",
        video_cover: "",
        video_length: ""
    ...
    getList() {
      this.loading=true;
      this.$get("/h5/dynamic/index").then(r => {
        // console.log(r);
        this.loading=false;
        if (r.http_code == 200) {
          let list = r.data;
    ...
    showTextarea(index, toUser) {
      if (this.content == "") {
        ...
        if (toUser.uid != this.userId && toUser.uid != this.hostUser.uid) {
    ...
    //删除动态Ï
    deleteDynamic(uid, idx) {
      // console.log(uid)
      //  console.log(idx)
      this.$get("/h5/dynamic/delete", {
        id: Number(uid)
      }).then(r => {
        if (r.http_code == 200) {
    ...
    //删除评论
    deleteComment(uid,pkid, idx) {
      this.$get("/h5/comment/delete", {
        id: Number(uid),
        dynamic_id:pkid
      }).then(r => {
        if (r.http_code == 200) {
          let list = this.activityList[idx].comment_list;
          for (let i = 0; i < list.length; i++) {
            if (list[i].id == uid) {
              list.splice(i, 1);
            }
          }
          this.$set(this.activityList[idx], "comment_list", list);
          this.$set(this.activityList[idx], "comment_count", list.length);
        }
      });
    },
  ...
  .edit-warp {
    ...
    .file-list {
      ...
      li {
        ...
        .icon-cancel {
          ...
          width: 20px;
          height: 20px;
          background: url("https://lib.liaoke.tv/lib/pc/static/img/center/icon-cancel.png")
            no-repeat center;
    ...
    .tar {
      ...
      .btn-file {
        ...
        width: 30px;
        height: 24px;
        background: url("https://lib.liaoke.tv/lib/pc/static/img/center/icon-img.png")
